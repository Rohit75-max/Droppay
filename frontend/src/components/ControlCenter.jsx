...
import {
  ...
  Monitor, Volume2, Play, Activity, Layout, Palette, Sparkles, Crown
} from 'lucide-react';
...
const ControlCenter = ({
  ...
  copyToClipboard, copiedType, triggerTestDrop, isTesting
}) => {
        ...
        <div className={`flex p-1.5 rounded-2xl border ${theme === 'dark' ? 'bg-black/40 border-white/5' : 'bg-slate-100 border-slate-200'}`}>
          {['overlay', 'mission', 'stickers'].map((tab) => (
            <button
              ...
              className={`px-8 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === tab
        ...
        <div className="grid grid-cols-1 gap-6">
            ...
            <div className={`p-8 md:p-10 rounded-[2.5rem] border transition-all ${getStudioStyle()}`}>
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-sm font-black uppercase italic tracking-widest">Alert Aesthetics</h3>
                </div>
                {isSavingAlerts && <Activity className="w-4 h-4 animate-spin text-[#10B981]" />}
              </div>
              <p className="text-[10px] font-bold text-slate-500 italic max-w-sm mb-6 leading-relaxed">Choose your live visual template. This instantly updates your OBS software and Donation page.</p>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <button
                  onClick={() => handleStyleSwitch('modern')}
                  className={`relative flex flex-col items-center justify-center gap-3 p-6 rounded-3xl border-2 transition-all overflow-hidden ${currentStyle === 'modern' ? 'border-[#10B981] bg-[#10B981]/10 shadow-[0_0_30px_rgba(16,185,129,0.15)]' : (theme === 'dark' ? 'border-white/5 bg-black/40 hover:border-white/20' : 'border-slate-200 bg-slate-50 hover:border-slate-300')}`}
                >
                ...
                <button
                  ...
                  className={`relative flex flex-col items-center justify-center gap-3 p-6 rounded-3xl border-2 transition-all overflow-hidden ${currentStyle === 'comic' ? 'border-[#FFDE00] bg-[#FFDE00]/10 shadow-[6px_6px_0px_0px_rgba(255,222,0,0.3)]' : (theme === 'dark' ? 'border-white/5 bg-black/40 hover:border-white/20' : 'border-slate-200 bg-slate-50 hover:border-slate-300')}`}
                >
                  <Zap className={`w-8 h-8 ${currentStyle === 'comic' ? 'text-[#FFDE00] fill-[#FFDE00]' : 'text-slate-400'}`} />
                  <span className={`text-[10px] font-black uppercase tracking-widest ${currentStyle === 'comic' ? 'text-[#FFDE00]' : 'text-slate-400'}`}>Comic</span>
                  {currentStyle === 'comic' && <div className="absolute top-4 right-4"><Check className="w-4 h-4 text-[#FFDE00]" /></div>}
                </button>

                <button
                  onClick={() => handleStyleSwitch('playful')}
                  className={`relative flex flex-col items-center justify-center gap-3 p-6 rounded-3xl border-2 transition-all overflow-hidden ${currentStyle === 'playful' ? 'border-[#FF5F6D] bg-[#FF5F6D]/10 shadow-[0_0_20px_rgba(255,95,109,0.25)]' : (theme === 'dark' ? 'border-white/5 bg-black/40 hover:border-white/20' : 'border-slate-200 bg-slate-50 hover:border-slate-300')}`}
                >
          ...
          {activeTab === 'stickers' && (
            <div className={`p-8 md:p-10 rounded-[2.5rem] border transition-all ${getStudioStyle()}`}>
              ...
              <div className="mt-8">
                <button onClick={savePartnerPack} disabled={isSavingStickers} className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg flex items-center justify-center gap-2 transition-all ${theme === 'dark' ? 'bg-[#10B981] text-black hover:bg-emerald-400' : 'bg-slate-900 text-white hover:bg-[#10B981]'}`}>
                  {isSavingStickers ? <Activity className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save Partner Pack
                </button>
              </div>
            </div>
          )}
        ...
        <div className={`p-8 rounded-[2.5rem] border sticky top-12 transition-all ${getStudioStyle()}`}>
              ...
              {isPreviewActive ? (
                <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 0.65, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} className="w-full h-full flex items-center justify-center origin-center">
                  <AlertPreview
                    ...
                    stylePreference={currentStyle} // Passes the selected style to Preview
                  />
                </motion.div>
              ) : (
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="flex justify-between items-center"><span className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2"><Volume2 className="w-3.5 h-3.5" /> Signal Volume</span><span className="text-[10px] font-black text-[#10B981]">{alertConfig?.volume || 50}%</span></div>
              <input type="range"
                min="0" max="100"
                ...
                className="w-full accent-[#10B981] cursor-pointer opacity-80"
              />
            </div>

            <div
              className={`p-4 rounded-[1.25rem] border-2 cursor-pointer transition-all flex justify-between items-center ${alertConfig?.ttsEnabled ? 'border-[#10B981] bg-[#10B981]/10 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : (theme === 'dark' ? 'border-white/5 bg-black/40 hover:border-white/20' : 'border-slate-200 bg-slate-50 hover:border-slate-300')}`}
              onClick={() => {
            ...
            >
              <div className="flex flex-col gap-1.5">
                ...
                <span className="text-[9px] font-bold text-slate-500 italic max-w-[200px] leading-relaxed">Node will synthesize and read incoming donor messages aloud.</span>
              </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={triggerLocalPreview}
                className={`flex-1 py-3 rounded-xl border-2 border-dashed font-black uppercase text-[9px] tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${theme === 'dark' ? 'border-white/10 text-slate-400 hover:border-[#10B981]' : 'border-slate-200 text-slate-500 hover:border-[#10B981]'}`}
              >
                <Monitor className="w-3.5 h-3.5" /> Local Preview
              </button>
              <button
                onClick={triggerTestDrop}
                disabled={isTesting}
                className={`flex-1 py-3 rounded-xl font-black uppercase text-[9px] tracking-[0.2em] transition-all flex items-center justify-center gap-2 shadow-lg ${theme === 'dark' ? 'bg-[#10B981] text-black hover:bg-emerald-400' : 'bg-slate-900 text-white hover:bg-[#10B981]'}`}
              >
                {isTesting ? <Activity className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5 fill-current" />} Test Overlay
              </button>
            </div>
          </div>
        </div>
