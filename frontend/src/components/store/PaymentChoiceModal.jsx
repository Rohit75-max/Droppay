                ...
                >
                    ...
                    <div className="p-8">
                        <div className="flex justify-between items-start mb-8">
                            <div>
                                <h2 className={`text-2xl font-black italic uppercase tracking-tighter ${theme === 'light' ? 'text-slate-900' : 'text-white'
                                    }`}>Acquire Asset</h2>
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#10B981] mt-1">Authorization Protocol Required</p>
                            </div>
                            <button onClick={onClose} className="p-2 hover:bg-black/5 rounded-full transition-colors">
                                <X className={`w-5 h-5 ${theme === 'light' ? 'text-slate-400' : 'text-slate-500'}`} />
                            </button>
                        </div>
                        ...
                        <div className={`mb-10 text-center p-6 rounded-2xl border ${theme === 'light'
                            ...
                            <span className="text-xs text-slate-500 uppercase font-bold tracking-widest">Asset Identified</span>
                            <h3 className="text-xl font-black uppercase text-[#10B981] mt-1">{itemName}</h3>
                            <div className="flex items-center justify-center gap-2 mt-2">
                                <IndianRupee className={`w-4 h-4 opacity-50 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`} />
                                <span className={`text-2xl font-black ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>{priceNum.toLocaleString()}</span>
                            </div>
                        </div>
