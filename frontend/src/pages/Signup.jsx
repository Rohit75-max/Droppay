...
import { User, Mail, Phone, Lock, ChevronRight, CheckCircle, Loader2, LogIn } from 'lucide-react';
          ...
          {step === 1 ? (
            <motion.div key="signup" exit={{ opacity: 0, x: -20 }}>
              <div className="flex items-center gap-3 mb-6 text-indigo-500">
                <LogIn className="w-6 h-6" /> {/* LogIn icon used */}
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                <div className="relative group">
                  ...
                  <input name="username" value={formData.username} onChange={handleChange} placeholder="Full Name" className="w-full bg-[#1a1a1a] border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:border-indigo-500 outline-none text-white" required />
                </div>
                ...
                <div className="relative group">
                  ...
                  <input name="streamerId" value={formData.streamerId} onChange={handleChange} placeholder="Unique-Slug (e.g. rohit-live)" className="w-full bg-[#1a1a1a] border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:border-indigo-500 outline-none text-white font-medium" required />
                </div>

                <div className="relative group">
                  <Mail className="absolute left-4 top-4 text-slate-600 w-5 h-5 group-focus-within:text-indigo-500 transition-colors" /> {/* Mail icon used */}
                  <input name="email" type="email" value={formData.email} onChange={handleChange} placeholder="Email Address" className="w-full bg-[#1a1a1a] border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:border-indigo-500 outline-none text-white" required />
                </div>

                <div className="relative group">
                  <Phone className="absolute left-4 top-4 text-slate-600 w-5 h-5 group-focus-within:text-indigo-500 transition-colors" /> {/* Phone icon used */}
                  <input name="phone" value={formData.phone} onChange={handleChange} placeholder="Phone Number" className="w-full bg-[#1a1a1a] border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:border-indigo-500 outline-none text-white" required />
                </div>

                <div className="relative group">
                  <Lock className="absolute left-4 top-4 text-slate-600 w-5 h-5 group-focus-within:text-indigo-500 transition-colors" /> {/* Lock icon used */}
                  <input name="password" type="password" value={formData.password} onChange={handleChange} placeholder="Password" className="w-full bg-[#1a1a1a] border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:border-indigo-500 outline-none text-white" required />
                </div>
